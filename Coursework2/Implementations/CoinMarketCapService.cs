...
namespace Coursework2.Realizations
{
    public class CoinMarketCapService : ICoinMarketCapFunctional
    {
        ...
        public async Task<CryptoCurrencyMetaData> GetCoinMarketCapMetaDataAsync()
        {
            try
            {
                string endpoint = string.Format($"v2/cryptocurrency/info?id=1");
                var queryParams = new Dictionary<string, object>
                {
        ...
        public async Task<CryptoCurrenciesListMetaData> GetCryptoCurrenciesListMetaAsync(int fiatId = 1)
        {
            string apiKey = await _keyFactory.GetNextValidAPIKeyAsync();
            try
            {
                string endpoint = $"v1/cryptocurrency/listings/latest?convert=ARS";
                var queryParams = new Dictionary<string, object>
                {
                    { "sort_dir", "desc" },
                    { "sort", "price" },
